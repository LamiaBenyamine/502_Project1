...
bootstrap Bootstrap t interval using a parametric bootstrap. We will use
$\alpha = 0.05$ as default in all calculations.
...
To calculate the Wald interval for $p$, we need to use sample's
probability $\hat{p}$, sample size $n$ and the confidence level
$1-\alpha$. The function of calculating Wald Interval is given below:

for (i in y)  # Handle special cases
  if (i == 0) {
    return(c(0, 0))  # CI for y = 0
  } else if (i == n) {
    return(c(1, 1))  # CI for y = n
  }
...
WaldCI <- function(y, n, alpha = 0.05){
  ...
  CI <- c("wald ci lower" = p_hat - qnorm(1-alpha/2)*sqrt((p_hat*(1-p_hat))/n),
    "wald ci upper" = p_hat + qnorm(1-alpha/2)*sqrt((p_hat*(1-p_hat))/n))
  contain <- (CI[1] < p) & (CI[2] > p)
  #Find width
  width <- CI[2] - CI[1]
  list(phat = p_hat, CIL = CI[1], CIU = CI[2], contain = contain, width = width, n = n)
}
```
...
When $y = 0$, we set the interval to be $[0,0]$ and When $y = n$, we set
the interval to be $[1,1]$. The function of calculating Clopper-Pearson
...
The function of finding Raw percentile interval and Bootstrap t interval
is given below:
...
We generated $N = 1000$ random samples from a binomial where sample size
$n$ varies across 15, 30, to 100 and true proportion $p$ varies from
0.01 to 0.99 (15 total values of p).
...
for (col_name in names(sample_data)) {
  ...
  p_value <- as.numeric(sub(".*p(\\d+\\.\\d+)", "\\1", col_name))
  ...
  waldci_results <- rbind(waldci_results, data.frame(
    ...
    True_P = p_value,
    Size = n_value,
    Coverage_rate = (ci[2] <= p_value) & (ci[3] >= p_value), # Check if the true p value is within the CI
    Miss_Above = !(ci[2] <= true_p),  # True p is above CI
    Miss_Below = !(ci[3] >= true_p),   # True p is below CI
    Width = ci[[3]] - ci[[2]] # Width of interval
  ))
}
Wald_Properties <- aggregate(cbind(Coverage_rate, Miss_Above, Miss_Below, Width) ~ Size, data = waldci_results, FUN = mean)
```

### Results from Adjusted Wald Simulation
```{r}
# Initialize a data frame to store the results
...
for (col_name in names(sample_data)) {
  ...
  adjwaldci_results <- rbind(adjwaldci_results, data.frame(
    ...
    True_P = p_value,
    Size = n_value,
    Coverage_rate = (ci[1] <= p_value) & (ci[2] >= p_value), # Check if the true p value is within the CI
    Miss_Above = !(ci[1] <= true_p),  # True p is above CI
    Miss_Below = !(ci[2] >= true_p),   # True p is below CI
    Width = ci[2] - ci[1] # Width of interval
  ))
}
...
Combining results from all the above methods.

```{r}

Results <- list(Wald_Properties, Adj_Wald_Properties, CP_Properties, Score_Properties)
```
